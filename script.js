...
function animateNumbers() {
  statNumbers.forEach(el => {
    ...
    const timer = setInterval(() => {
      ...
      if (Number.isInteger(target)) el.textContent = Math.floor(current);
...
const aboutObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => { if (entry.isIntersecting) { animateNumbers(); aboutObserver.unobserve(entry.target); } });
}, { threshold: 0.3 });
